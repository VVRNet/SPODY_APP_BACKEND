...
const resBody = Type.Object({
  game: Type.Array(
    ...
    { description: '초대받은 게임 리스트' },
  ),
})
type resBodyType = Static<typeof resBody>

export const ApiGameGetInvited: RegisterApi = (
  app: FastifyInstance,
  url: string,
...
`,
    ...
    handler: async (request, reply) => {
      ...
      // 현재 참여 정보 확인
      let listJoinedGame: WithId<SchemaGame>[]
      try {
        listJoinedGame = await CollGame.find({
          members: { $elemMatch: { id: userId, type: userType, status: 'inviting' } },
        }).toArray()
      } catch (e) {
      ...
      // API 응답
      reply.send({
        game: listJoinedGame.map((g) => ({
          id: g._id.toHexString(),
          hostName: g.host.name,
          hostNameOrg: g.host.orgName,
          players: g.members.length - (g.host.role === 'play' ? 0 : 1),
        })),
      })
    },
  })
