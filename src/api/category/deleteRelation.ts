...
const reqBody = Type.Object({
  ...
  keywordId: Type.Array(
    ...
    { description: '연결삭제할할 키워드 id 배열' },
  ),
})
type reqBodyType = Static<typeof reqBody>

export const ApiCategoryRelationDelete: RegisterApi = (
  app: FastifyInstance,
  url: string,
...
`,
    ...
    handler: async (request, reply) => {
      ...
      // 키워드에 연관 데이터 추가
      try {
        const a = await CollKeyword.updateMany(
          {
            _id: { $in: keywordIds },
          },
          {
        ...
        console.log('matchedCount:' + a.matchedCount)
        console.log('upsertedCount:' + a.upsertedCount)
      } catch (e) {
