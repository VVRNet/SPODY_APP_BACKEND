...
export const ApiQuizRecommend: RegisterApi = (
  app: FastifyInstance,
  url: string,
...
`,
    ...
    handler: async (request, reply) => {
      ...
      // gpt에게 퀴즈 질의
      // const prompt = `Give 1 quiz to students.
...
The quiz consists of the following contents.
...
`
      ...
      const quizs = await Promise.all(
        [...Array(10)].map(async (_, i) => {
          ...
          //   console.log(prompt)
          // }
          await new Promise((f) => setTimeout(f, i * 200))
          let resRaw = await OpenAiAsk(prompt, true)

          try {
            const raw = JSON.parse(resRaw)
            ...
            throw new ExError('gpt answer is wrong format', {
              type: 'int',
              code: 'OPENAI_WRONG',
              info: { message: prompt, response: JSON.stringify(resRaw), questionIndex: i },
              err: e,
            })
          }
        }),
      )
      let res: resBodyType = { quiz: quizs }
      // 언어별 후보정
      const languageArr = {
