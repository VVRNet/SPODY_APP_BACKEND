...
const reqBody = Type.Object({
  email: Type.String({
    description: '암호 초기화 및 인증코드 받고자 하는 메일주소',
    format: 'email',
  }),
})
...
export const ApiAuthPwMailReq: RegisterApi = (
  app: FastifyInstance,
  url: string,
  ...
  app.route<{ Body: reqBodyType; Reply: resBodyType }>({
    ...
    schema: {
      tags: ['인증'],
      summary: '암호초기화 flow 1 : 검증코드 이메일 발송 요청. 인증코드는 10분간 유효',
      description: `[ 400에러 코드 ]
      
      . AUTH_EMAIL_NOTFOUND : 존재하지 않는 계정
`,
      ...
      while (true) {
        ...
        try {
          ...
          if (
            ...
            throw new ExError('fail to send email', {
              type: 'int',
              code: 'DB_INSERT_CODE',
              err: e,
            })
