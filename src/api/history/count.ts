...
export const ApiHistoryCount: RegisterApi = (
  app: FastifyInstance,
  url: string,
  ...
  app.route<{ Querystring: reqQueryType; Reply: resBodyType }>({
    ...
    schema: {
      tags: ['기록'],
      summary: '해당 기간 날짜 별 푼 퀴즈의 개수(count) 만 리턴한다',
...
`,
    ...
    handler: async (request, reply) => {
      ...
      const dateCountArr: { date: number; count: number }[] = []
      for (const date of histories.map((h) => h.recordAt)) {
        ...
        if (dateElem == null) {
          dateCountArr.push({ date: dateKey, count: 1 })
        } else {
          dateElem.count++
        }
      }
      ...
      const resData: { date: number; count: number }[] = []
      for (let date = resDateFrom; date <= resDateTo; date = date + 86400) {
        resData.push({
          date: date,
          count: dateCountArr.find((c) => c.date === date)?.count ?? 0,
        })
      }

      // API 응답
      reply.send({ list: resData })
    },
  })
}
